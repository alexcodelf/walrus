...
// The WorkflowStepTemplateFunc type is an adapter to allow the use of ordinary
// function as WorkflowStepTemplate mutator.
type WorkflowStepTemplateFunc func(context.Context, *model.WorkflowStepTemplateMutation) (model.Value, error)

// Mutate calls f(ctx, m).
func (f WorkflowStepTemplateFunc) Mutate(ctx context.Context, m model.Mutation) (model.Value, error) {
	if mv, ok := m.(*model.WorkflowStepTemplateMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *model.WorkflowStepTemplateMutation", m)
}
