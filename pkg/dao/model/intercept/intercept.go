...
// The WorkflowStepTemplateFunc type is an adapter to allow the use of ordinary function as a Querier.
type WorkflowStepTemplateFunc func(context.Context, *model.WorkflowStepTemplateQuery) (model.Value, error)

// Query calls f(ctx, q).
func (f WorkflowStepTemplateFunc) Query(ctx context.Context, q model.Query) (model.Value, error) {
	if q, ok := q.(*model.WorkflowStepTemplateQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *model.WorkflowStepTemplateQuery", q)
}

// The TraverseWorkflowStepTemplate type is an adapter to allow the use of ordinary function as Traverser.
type TraverseWorkflowStepTemplate func(context.Context, *model.WorkflowStepTemplateQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseWorkflowStepTemplate) Intercept(next model.Querier) model.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseWorkflowStepTemplate) Traverse(ctx context.Context, q model.Query) error {
	if q, ok := q.(*model.WorkflowStepTemplateQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *model.WorkflowStepTemplateQuery", q)
}
...
func NewQuery(q model.Query) (Query, error) {
	...
	case *model.WorkflowStepTemplateQuery:
		return &query[*model.WorkflowStepTemplateQuery, predicate.WorkflowStepTemplate, workflowsteptemplate.OrderOption]{typ: model.TypeWorkflowStepTemplate, tq: q}, nil
	default:
