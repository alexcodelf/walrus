...
func (wseci *WorkflowStepExecutionCreateInput) Model() *WorkflowStepExecution {
	...
	if wseci.WorkflowStep != nil {
		_wse.WorkflowStepID = wseci.WorkflowStep.ID
	}
...
func (wsedi *WorkflowStepExecutionDeleteInputs) ValidateWith(ctx context.Context, cs ClientSet, cache map[string]any) error {
	...
	if wsedi.WorkflowStep != nil {
		if err := wsedi.WorkflowStep.ValidateWith(ctx, cs, cache); err != nil {
			...
			q.Where(
				workflowstepexecution.WorkflowStepID(wsedi.WorkflowStep.ID))
		}
	}
...
func (wseqi *WorkflowStepExecutionQueryInput) ValidateWith(ctx context.Context, cs ClientSet, cache map[string]any) error {
	...
	if wseqi.WorkflowStep != nil {
		if err := wseqi.WorkflowStep.ValidateWith(ctx, cs, cache); err != nil {
			return err
		} else {
			q.Where(
				workflowstepexecution.WorkflowStepID(wseqi.WorkflowStep.ID))
		}
	}
...
func (wseui *WorkflowStepExecutionUpdateInputs) ValidateWith(ctx context.Context, cs ClientSet, cache map[string]any) error {
	...
	if wseui.WorkflowStep != nil {
		if err := wseui.WorkflowStep.ValidateWith(ctx, cs, cache); err != nil {
			...
			q.Where(
				workflowstepexecution.WorkflowStepID(wseui.WorkflowStep.ID))
		}
	}
...
func ExposeWorkflowStepExecution(_wse *WorkflowStepExecution) *WorkflowStepExecutionOutput {
	...
	if _wse.Edges.WorkflowStep != nil {
		...
		wseo.WorkflowStep = &WorkflowStepOutput{
			ID: _wse.WorkflowStepID,
		}
	}
