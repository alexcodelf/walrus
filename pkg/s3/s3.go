...
func open(ctx context.Context, cfg Config, rt http.RoundTripper) (*Backend, error) {
	var logger = log.WithName("s3")

	logger.Debugf("open, config %#v", cfg)

	if cfg.KeyID == "" && cfg.Secret != "" {
		return nil, errors.New("unable to open S3 backend: Key ID ($AWS_ACCESS_KEY_ID) is empty")
	} else if cfg.KeyID != "" && cfg.Secret == "" {
		return nil, errors.New("unable to open S3 backend: Secret ($AWS_SECRET_ACCESS_KEY) is empty")
	}
...
func getCredentials(cfg Config) (*credentials.Credentials, error) {
	...
	if c.SignerType == credentials.SignatureAnonymous {
		logger.Debugf("anonymous credentials detected, skipping assume role")
	}
...
func (be *Backend) Save(ctx context.Context, rd io.Reader) error {
	// TODO
	objName := ""
	...
	if err == nil && info.Size != rd.Length() {
		return fmt.Errorf("wrote %d bytes instead of the expected %d bytes", info.Size, rd.Length())
	}
