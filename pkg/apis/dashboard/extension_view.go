...
type (
	CollectionRouteGetLatestResourceRunRequest struct {
		_ struct{} `route:"GET=/latest-resource-runs"`

		Context *gin.Context
	}

	CollectionRouteGetLatestResourceRunResponse = []*model.ResourceRunOutput
)

func (r *CollectionRouteGetLatestResourceRunRequest) SetGinContext(ctx *gin.Context) {
	r.Context = ctx
}
