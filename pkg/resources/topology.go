...
type ResourceNode struct {
	...
	Resource *walruscore.Resource `json:"resource"`
	Children []*ResourceNode      `json:"children"`
}

// NewResourceNodes returns a list of resource nodes from the given resources.
func NewResourceNodes(resources walruscore.Resources) ([]*ResourceNode, error) {
	var (
		resourceMap = make(map[string]*walruscore.Resource)
	...
	for i := range resources {
		...
		if len(res.Status.Dependencies) > 0 {
			for _, name := range res.Status.Dependencies {
...
func newResourceNodeFromDependency(
	...
	logger := klog.Background().WithName("resource").WithName("topology")
	...
	for name, dependencies := range dependencyMap {
		...
		for _, dependencyName := range dependencies {
			if _, ok := nodeMap[dependencyName]; ok {
				...
				logger.Infof("name: %s, %v, dependency %s not found", name, dependencies, dependencyName)
			}
		}
	}
...
func TopologicalSortResources(resources walruscore.Resources) (walruscore.Resources, error) {
	var sortedResources walruscore.Resources
...
func ReverseTopologicalSortResources(resources walruscore.Resources) (walruscore.Resources, error) {
	var sortedResources walruscore.Resources
