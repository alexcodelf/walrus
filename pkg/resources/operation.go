...
func Upgrade(ctx context.Context, mc model.ClientSet, entity *model.Resource, opts Options) error {
...
func Delete(ctx context.Context, mc model.ClientSet, entity *model.Resource, opts DeleteOptions) (err error) {
	...
	// Delete the resource, mark status to deleting.
...
func Start(ctx context.Context, mc model.ClientSet, entity *model.Resource, opts Options) error {
	if IsInactive(entity) {
		return fmt.Errorf("cannot start resource %q: in %q status", entity.Name, entity.Status.SummaryStatus)
	}
...
func ApprovePlanedResource(
	...
	run, err := dao.GetResourceLatestRun(ctx, mc, entity.ID)
	if err != nil {
		return err
	}

	if !runstatus.IsStatusPlanned(run) {
		return fmt.Errorf("resource run %s is not planned", run.ID)
	}
...
func CollectionUpgrade(ctx context.Context, mc model.ClientSet, entities model.Resources, opts Options) error {
	...
	for envID := range groupedResources {
		...
		for _, entity := range envResources {
			err = upgrade(ctx, mc, entity, opts)
			if err != nil {
				return err
			}
		}
...
func CollectionCreate(
	...
	for envID := range groupedResources {
		...
		for i := range envResources {
			...
			entity, err = Create(ctx, mc, entity, opts)
			if err != nil {
				return nil, err
			}
...
func errorHandler(mc model.ClientSet, res *model.Resource, run *model.ResourceRun, ct status.ConditionType, err error) {
	...
	updateErr := UpdateStatus(ctx, mc, res)
	if updateErr != nil {
...
func PerformResource(ctx context.Context, mc model.ClientSet, dp deptypes.Deployer, resourceID object.ID) error {
	run, err := dao.GetResourceLatestRun(ctx, mc, resourceID)
	if err != nil {
		return err
	}
