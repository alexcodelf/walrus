...
func (m *Manager) SetRunPlan(ctx context.Context, run *walruscore.ResourceRun, plan []byte) error {
	fileName := walruscore.ResourceRunPlanFileName(run)
...
func (m *Manager) GetRunPlan(ctx context.Context, run *walruscore.ResourceRun) ([]byte, error) {
	fileName := walruscore.ResourceRunPlanFileName(run)
	bucketName := m.config.Bucket
...
func (m *Manager) DeleteRunPlan(ctx context.Context, run *walruscore.ResourceRun) error {
	fileName := walruscore.ResourceRunPlanFileName(run)
	bucketName := m.config.Bucket
