...
const (
	// _planCommands the commands to plan deployment of the resource run.
	_planCommands = "terraform init -no-color && terraform plan -no-color"
...
func CreateJob(ctx context.Context, clientSet *kubernetes.Clientset, opts JobCreateOptions) error {
	var (
		...
		name                          = getK8sJobName(_jobNameFormat, opts.Type.String(), opts.ResourceRunID)
...
func getPodTemplate(runID, configName string, opts JobCreateOptions) corev1.PodTemplateSpec {
	...
	case types.RunTaskTypePlan:
		deployCommand += _planCommands + varfile
	...
	return corev1.PodTemplateSpec{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				types.LabelWalrusResourceRunID: runID,
			},
		},
		Spec: corev1.PodSpec{
			HostNetwork:        true,
