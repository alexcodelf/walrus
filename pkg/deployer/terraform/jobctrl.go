...
const (
	...
	// _applicationRunIDLabel pod template label key for application run id.
	_applicationRunIDLabel = "walrus.seal.io/application-run-id"
...
func (r JobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	err = r.syncApplicationRunStatus(ctx, job)
...
// syncApplicationRunStatus sync the application run status.
func (r JobReconciler) syncApplicationRunStatus(ctx context.Context, job *batchv1.Job) (err error) {
	appRunID, ok := job.Labels[_applicationRunIDLabel]
	if !ok {
		// Not a deployer job.
		return nil
	}
	...
	// If the application run status is not running, then skip it.
	...
	if err != nil {
		r.Logger.Error(err, "failed to get job pod logs", "application-run", appRunID)
		record = err.Error()
	}

	if job.Status.Succeeded > 0 {
		r.Logger.Info("succeed", "application-run", appRunID)
	}

	if job.Status.Failed > 0 {
		r.Logger.Info("failed", "application-run", appRunID)
...
func getPodTemplate(applicationRunID, configName string, opts JobCreateOptions) corev1.PodTemplateSpec {
	var (
	...
	return corev1.PodTemplateSpec{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				_applicationRunIDLabel: applicationRunID,
			},
		},
		Spec: corev1.PodSpec{
			HostNetwork:        true,
...
// getK8sJobName returns the kubernetes job name for the given application run id.
func getK8sJobName(format, jobType, applicationRunID string) string {
	return fmt.Sprintf(format, jobType, applicationRunID)
}
