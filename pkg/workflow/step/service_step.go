...
# Get latest run id
runResponse=$(curl -s "$commonPath/resources/$resourceName/runs?page=1&perPage=1&sort=-createTime" -X GET -H "Authorization: Bearer $token" $tlsVerify)
runID=$(echo $runResponse | jq -r '.items[0].id')

# Watch service logs until the service finished.
curl -o - -s "$commonPath/resources/$resourceName/runs/$runID/log?jobType=apply&watch=true" -X GET -H "Authorization: Bearer $token" $tlsVerify --compressed

# Check run status. wait until run status is ready.
timeout=30
factor=1
while true; do
	statusResponse=$(curl -s "$commonPath/resources/$resourceName/runs/$runID" -X GET -H "Authorization: Bearer $token" $tlsVerify)
