...
func (s *ArgoWorkflowClient) Resume(ctx context.Context, opts ResumeOptions) error {
	...
	approvalSpec, err := types.NewWorkflowStepApprovalSpec(opts.WorkflowStepExecution.Spec)
	if err != nil {
		return err
	}
	...
	// Update workflow step execution approval spec.
	err = s.mc.WorkflowStepExecutions().UpdateOne(opts.WorkflowStepExecution).
		SetSpec(approvalSpec.ToSpec()).
		Exec(ctx)
	if err != nil {
		return err
	}
...
func (s *ArgoWorkflowClient) updateWorkflowExecutionToken(
	...
	secret, err := clientSet.CoreV1().Secrets(types.WalrusWorkflowNamespace).
